...
后续在 Beak host 中落地 Gateway 时，建议按当前代码边界拆成以下改造项。
...
- 服务启动后可按数据库中 active accounts 自动恢复 connector runtime。
- 服务关闭时统一 cancel runtime，并保存最后状态。

当前 `server.New` 直接创建 `store.NewMemoryStore()`，适合作为原型。生产落地时应将 store 创建抽象到配置层，避免 Gateway 只能运行在内存模式。
...
- 当前 `CreateMessage` 只写 message 和 event，不会让 agent 产生回复。
...
- 当前 stream 建连后只输出历史事件和 heartbeat。
- 正式 Gateway 需要 `SessionEvent` 实时 fanout，否则 connector 只能通过 reconnect 轮询 agent 输出。
- event hub 可按 `session_uuid` 管理 subscribers；`CreateMessage` 写入 event 后 publish。
- `last_event_uuid` 仍必须保留，作为断线续传和 dedupe 的可靠边界。
...
5. Gateway 调用现有 routing 或默认 agent 选择逻辑，确定本 session 的 agent participant。
...
6. 补 message sender participant 校验和 reply 校验。
7. 补 inbound 到 agent 的 application service，不让 connector 直接绕过 Beak 消息链路。
8. 注册 Weixin connector，完成扫码、account start、getupdates、文本 inbound、固定回复或 agent 回复 outbound。
9. 注册 Lark 和 DingTalk connector，完成 credential account、webhook inbound、文本 outbound。
10. 补数据库 SQL/migration 文档，与当前 Go model 对齐。
11. 补 event hub，让 stream 能实时推送新增 `SessionEvent`。
12. 补失败重试、运行时恢复、指标和审计日志。
